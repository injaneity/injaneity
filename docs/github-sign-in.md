...
run `npm run build && npm run test:site && npm run typecheck && npm run lint`. the tests include a mocked github provider inside cloudflare's actual runtime, covering request construction, sessions, and redirect rejection. use `redirect: 'manual'` and reject non-success responses: workerd rejects `redirect: 'error'` before making the request, even though node accepts it.

production credentials are configured on the cloudflare pages project `injaneity`. real github sign-in, owner-only `edit · create`, editor access, and signed-out 404 protection were verified on 2026-09-23. callback failures log only fixed stage names, error categories, status codes, and allowlisted provider errors; never log request URLs, cookies, credentials, provider bodies, or raw exception messages.
