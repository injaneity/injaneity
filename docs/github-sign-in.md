...
run `npm run build && npm run test:site && npm run typecheck && npm run lint`. the auth tests use a mocked github provider. real local github sign-in has also been verified with the development app. production credentials are configured on the cloudflare pages project `injaneity`; the live session endpoint, oauth redirect, and signed-out editor protection are verified. the first production browser sign-in still needs the owner's github authorization.
